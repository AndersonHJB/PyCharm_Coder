...
player = Greature(300,user_player_name1)   #②如果需要填入初始值的话，那我们就是需要在class（类）里面提前定义这个值。/预设的地方。
...
while player.not_dead() and enemy.not_dead(): #①如何判断呢？那我们就需要你个初始换的声明值（血条）
	...
	if user_input != 'A' or 'B' or 'D':
		...
		player.being_attack(enemy_attack_value + enemy2_attack_value)      #玩家所承受的攻击值是由敌人带来的



	elif user_input == 'B':
		player_attack_value = player.attack2()*10
		enemy_attack_value  = enemy.attack2()
		enemy2_attack_value  = enemy2.attack2()
		enemy.being_attack2(player_attack_value)
		enemy2.being_attack2(player_attack_value)
		player.being_attack2(enemy_attack_value + enemy2_attack_value)


	elif user_input == 'D':                       #玩家选择防守
		enemy_attack_value = enemy.attack()*0.1   #因为是防守所以呢，就可以在原本的攻击值上乘于十分之一（也就是0.1）
		enemy2_attack_value = enemy2.attack2()*0.5
		player.being_attack(enemy_attack_value + enemy2_attack_value)   #玩家要承担敌人攻击的数值
...
if player.not_dead():
	print("You win!")
	print('是否进入BOSS模式，进入请输入1，否则请输入2:')
	while 
else:
	print("You lose!")
