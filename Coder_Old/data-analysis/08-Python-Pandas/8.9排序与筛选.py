...
# 筛选：行（索引、值、行数）、列、行列
# print(data.iloc[0:100])  # 0-100行数据
# print(data.loc[data['average'] == 9.7])
# print(data[['genre', 'title']])
# print(data.loc[data['average'] == 9.7, ['genre', 'title']])  # 满足行列双重条件

print(data[['title', 'average', 'votes']].sort_values(['average'], ascending=False).head(10))
