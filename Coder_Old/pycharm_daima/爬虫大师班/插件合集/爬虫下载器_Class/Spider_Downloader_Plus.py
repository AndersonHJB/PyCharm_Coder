...
class Spider_Downloader(object):
	def __init__(self, url = None, headers = None, layout = None, file_name = None):
		self.url = self.isnone(url)
		self.layout = self.isnone(layout)
		self.file_name = self.isnone(file_name)
		self.session = requests.Session()
		self.session.headers = self.isnone(headers)

	def isnone(self, variat):
		if variat is None:
			variat = None
			return variat
		variat = variat
...
if __name__ == '__main__':
	img_download = Spider_Downloader(url = url)
	img_download.main()
	# requests.get('www.aiyc.top')