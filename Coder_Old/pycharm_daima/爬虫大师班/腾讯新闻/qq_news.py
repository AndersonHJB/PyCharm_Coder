"""
# -*- coding: utf-8 -*-
# @Author：AI悦创 @DateTime ：2019/9/26  10:58 @Function ：功能  Development_tool ：PyCharm
		...
		for i in data:
			...
			if text:
				for text_main in text:
					if text_main:
						for text in text_main.stripped_strings:
							# print(text)
						# print(type(text_main.get_text()))
							data_list.append(text_main.get_text())
							data_tuple = tuple(l)
						# data_list2.append(data_tuple)
			# with open('QQ新闻2019.csv') as f:
			# 	print(data_list2)
			#
				# f.write(text_main.get_text())
				# f.write('\n')
				# f_csv = csv.writer(f)
				# f_csv.writerows(data_list2)
	# 使用正则表达式
	# def spider_plus(self, data):
	# 	for i in data:
	# 		html = self.session.get(i).text
