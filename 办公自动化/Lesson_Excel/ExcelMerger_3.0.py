...
class ExcelMerger(object):
	...
	def __init__(self, src_path=".", res_path="默认路径.xls", sheet_header=[], result_sheeet="合并结果",
	             duplicate_removal=False, whether_to_add_filename=True):
		# 指定要合并 excel 的路径
	...
	# def filename(self, rows):
	# 	"""实现 filename 出现的次数"""
	# 	filename_list = []
	# 	if self.whether_to_add_filename:
	# 		for i in range(self.whether_to_add_filename):
	# 			filename_list.append(i)
	# 	for _ in range(rows - self.whether_to_add_filename):
	...
	def read_excel_full(self, filename, sheet):
		...
		for row in range(1, rows):
			temp = []
			if self.whether_to_add_filename: temp.append(filename)
